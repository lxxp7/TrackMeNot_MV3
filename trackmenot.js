...
TRACKMENOT.TMNSearch = (function () {
  ...
  function getElementsByAttrValue(dom, nodeType, attrName, nodeValue) {
    ...
    for (var i = 0; i < outlines.length; i++) {
      if (
        outlines[i].hasAttribute(attrName) &&
        outlines[i].getAttribute(attrName) === nodeValue
      )
        return outlines[i];
    }
    return null;
    ...
    list: [
      {
        ...
        enabled: true,
        regexmap:
          "^(https?:\/\/[a-z]+\.google\.(co\\.|com\\.)?[a-z]{2,3}\/(search){1}[\?]?.*?[&\?]{1}q=)([^&]*)(.*)$",
      },
      {
        id: "yahoo",
        name: "Yahoo! Search",
        urlmap:
          ...
          getYahooId() +
          "?ei=UTF-8&fr=sfp&fr2=sfp&p=|&fspl=1",
        enabled: true,
        regexmap:
          "^(https?:\/\/[a-z.]*?search\.yahoo\.com\/search.*?p=)([^&]*)(.*)$",
      ...
      },
      {
        ...
        enabled: false,
        regexmap:
          "^(https?:\/\/[a-z0-9.]*?search\.aol\.com\/aol\/search\?.*?q=)([^&]*)(.*)$",
    ...
    if (tmn_tab_id === -1) return;
      ...
      api.tabs.remove(tmn_tab_id, function () {
        if (api.runtime.lastError) {
          cout(
            "Can't kill tab due to error : " + api.runtime.lastError.message,
          );
        }
      });
    }
  ...
  function monitorBurst() {
    api.webNavigation.onCommitted.addListener(function (e) {
      ...
      if (tmn_tab_id === -1 || tab_id !== tmn_tab_id) {
        debug(
          "Worker find a match for url: " + url + " on engine " + eng + "!",
        );
        ...
        if (engine && engine.urlmap !== asearch) {
          ...
          log(logEntry);
          debug(
            "Updated url fr search engine " + eng + ", new url is " + asearch,
          );
        }
      }
    });
  ...
  function randomQuery() {
    // Only consider feed types that actually have entries. RSS/DHS may still be
    // empty (fetch failed or in flight), so filtering here avoids crashing.
    var typeoffeeds = Object.keys(TMNQueries).filter(function (k) {
      var v = TMNQueries[k];
      return v && v.length > 0;
    });
    if (typeoffeeds.length === 0) return trim(randomElt(zeit_queries));

    var queries = [];
    for (
      var attempt = 0;
      attempt < 10 && (!queries || queries.length === 0);
      attempt++
    ) {
      var qtype = randomElt(typeoffeeds);
      if (qtype !== "zeitgeist" && qtype !== "extracted") {
        var entry = randomElt(TMNQueries[qtype]);
        queries = entry && entry.words ? entry.words : [];
      } else {
        queries = TMNQueries[qtype];
      }
    }
    if (!queries || queries.length === 0) queries = zeit_queries;

    var term = trim(randomElt(queries));
    if (!term || term.length < 1) return trim(randomElt(zeit_queries));
    return term;
  }
  ...
  function validateFeeds(param) {
    ...
    for (var i = 0; i < feeds.length; i++) doRssFetch(feeds[i]);
    saveOptions();
  }
  ...
  function queryOk(a) {
    for (let i = 0; i < skipex.length; i++) {
      if (skipex[i].test(a)) return false;
    }
    return true;
  }
  ...
  function addQuery(term, queryList) {
    ...
    if (isBlackList(term)) return false;

    if (!term || term.length < 3 || queryList.indexOf(term) > 0) return false;

    if (term.indexOf('""') > -1 || term.indexOf("--") > -1) return false;

    // test for negation of a single term (eg '-prison')
    if (term.indexOf("-") === 0 && term.indexOf(" ") < 0) return false;

    if (!queryOk(term)) return false;
  ...
  function doRssFetch(feedUrl) {
    ...
    fetch(feedUrl)
      ...
      .then((str) => {
        // Extraction manuelle des titres avec une regex (compatible Service Worker)
        var titles = [];
        var match;
        var regex = /<title>(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?<\/title>/g;
        ...
        // Simulation d'un mini-document requis par addRssTitles
        var mockDoc = {
          getElementsByTagName: function (tagName) {
            if (tagName === "title") {
              // addRssTitles() reads node.firstChild.nodeValue, so mimic that shape
              return titles.map((t) => ({ firstChild: { nodeValue: t } }));
            }
            return [];
          },
        };
  ...
  function getSubQuery(queryWords) {
    ...
    for (var k = 0; k < queryWords.length; k++) {
      ...
      if (randomArray.indexOf(randomIndex) < 0) randomArray.push(randomIndex);
    }
    randomArray.sort();
    ...
    incQuery += queryWords[randomArray[k]];
    if (incQueries) incQueries.push(trim(incQuery));
  }

  function getQuery() {
    var term = randomQuery();
    if (term.indexOf("\n") > 0) {
      // yuck, replace w' chomp();
      while (true) {
        for (var i = 0; i < term.length; i++) {
          if (term.charAt(i) === "\n") {
            term =
              term.substring(0, i) + " " + term.substring(i + 1, term.length);
            continue;
          }
        }
        break;
  ...
  function doSearch() {
    ...
    if (incQueries && incQueries.length > 0) sendQuery(null);
    else {
      newquery = getQuery();
      ...
      if (queryWords.length > 3) {
        ...
        if (incQueries && incQueries.length > 0) newquery = incQueries.pop();
      }
      sendQuery(newquery);
    }
  }
  ...
  function sendQuery(queryToSend) {
    ...
    if (queryToSend === null) {
      if (incQueries && incQueries.length > 0) queryToSend = incQueries.pop();
      else {
    ...
    if (tmn_options.useTab) {
      ...
      api.cookies.getAll({ url: queryURL }, function (cookies) {
        sendBackgroundQuery(cookies, queryURL, queryToSend);
      });
    }
  }

  function sendBackgroundQuery(cookies, queryURL, queryToSend) {
    var cookies_string = cookies
      .map(function (x) {
  ...
  function rescheduleOnError() {
    ...
    if (tmn_options.enabled) scheduleNextSearch(pauseAfterError);
  }
  ...
  function reschedule() {
    ...
    if (isBursting()) {
      ...
      // Not bursting, schedule per usual
      tmn_mode = "timed";
      scheduleNextSearch(delay);
    }
  }
  ...
  function scheduleNextSearch(delay) {
    ...
    if (delay > 0) {
      if (!isBursting()) {
        ...
        // just simple randomize during a burst
    ...
    if (isBursting()) engine = burstEngine;
      ...
      engine = chooseElt(
        tmn_engines.list.filter(function (x) {
          return x.enabled;
        }),
      ).id;
    debug("NextSearchScheduled on: " + engine);

    // Correction ici : suppression de "window."
    clearTimeout(tmn_errTimeout);
    tmn_errTimeout = setTimeout(rescheduleOnError, delay * 3);
    clearTimeout(tmn_searchTimer);
    tmn_searchTimer = setTimeout(doSearch, delay);
  }
  ...
  function startTMN() {
    ...
    api.windows.onRemoved.addListener(function () {
      if (!tmn_options.saveLogs) api.storage.local.set({ logs_tmn: "" });
    });
  }
  function getTMNHTML() {
    var req = {};
      ...
      case "pageLoaded":
        if (!tmn_hasloaded) {
          ...
          if (tmn_options.sim_clicks && Math.random() < 0.3) {
            ...
            setTimeout(sendClickEvent, time);
          } else {
            setTimeout(getTMNHTML, 1000);
          }
        }
        sendResponse({});
        break;
  ...
  return {
    ...
    _logStorageChange: function (items) {
      if ("options_tmn" in items) updateOptions(items.options_tmn.newValue);
      if ("engines_tmn" in items) setEngines(items.engines_tmn.newValue);
    },
    ...
    _restoreTMN: function (items) {
      ...
      if (!items["tab_url"]) {
        ...
        api.tabs.query(
          { pinned: false, url: items["tab_url"] },
          function (tabs) {
            if (tabs.length !== 0) {
...
TRACKMENOT.TMNSearch._getStorage(
  [
    "options_tmn",
    "gen_queries",
    "engines_tmn",
  ...
  TRACKMENOT.TMNSearch._restoreTMN,
);
api.storage.onChanged.addListener(TRACKMENOT.TMNSearch._logStorageChange);

// MV3: the service worker is terminated after ~30s of inactivity, which would
// drop the pending setTimeout timers used to schedule searches. Periodically
// touching a chrome API resets the idle timer and keeps the worker alive.
